...
if (scenario_id %in% unique(scenario_df$scenario_id)) {
  out <- NULL
  
  for (stochastic_num in 1:30) {
    for (edge_iter in 1:5) {
      
      temp <- fread(paste0(results_folder, sens, "/sim_", stochastic_num, "_scenario_", scenario_id, "_edge_", edge_iter, "_out_full.csv"))
      ...
      temp <- temp[hiv==1 & cd4 < 500 & cd4>=200 & art == 1, hiv_qol:=.98]
      temp <- temp[hiv==1 & cd4 < 500 & cd4>=200 & (art == 0 | is.na(art)), hiv_qol:=0.867]
      temp <- temp[hiv==1 & cd4 < 200 & art == 1, hiv_qol:=0.799]
      temp <- temp[hiv==1 & cd4 < 200 & (art == 0 | is.na(art)), hiv_qol:=0.707]
      
      ## PWID Utility
      temp <- temp[current_pwid == 1, pwid_qol:=0.68]
      temp <- temp[current_pwid == 0, pwid_qol:=0.82]
    ...
    fwrite(out, paste0(summaries_folder, sens, "/out_full_scenario_", scenario_id, ".csv"), na = "", row.names = F)
}
